...
<p>Tiny ring resonators can produce "squeezed" light: light whose noise, measured the right way, drops below the usual quantum limit, a resource for precision measurement and quantum computing. sqzcomb predicts how much squeezing a detector will actually see from such a ring, including everything that eats it along the way: losses, imperfect detectors, and how the light is extracted. It covers pairs of coupled rings, which its test suite drives past the well-known single-ring extraction limit, and it can also quantify the quantum entanglement between pairs of comb lines. Its tests check it against textbook formulas and independent calculations. Latest release (0.13.0): it now also works above the oscillation threshold, including an exact quantum calculation for light that is no longer Gaussian; it adds classical noise from the ring and the pump laser, pulsed pumps, and coupled rings with different line spacings; and it fixes the angle at which some noise readings were taken.</p>
...
<p>Finding a good single-photon emitter (the light source behind many quantum technologies) means measuring hundreds of candidate spots, most of them useless. SPARQ automates that screening. It simulates the standard photon-correlation measurement faithfully, analyzes real measured data with honest uncertainties, and decides on the fly how long to keep measuring each spot, stopping the moment the evidence is sufficient instead of waiting out a fixed dwell time. It also includes machine-learning agents that learn the screening strategy itself. The physics core is tested against exact reference calculations. It ships built-in parameter ranges for four kinds of emitter (NV and SiV centres in diamond, and emitters in hBN and GaN) and lets you add your own. Latest release (0.10.0): it now fits dips of other shapes (several dark states, or an emitter driven directly by a laser) and says when a shape does not fit; uses the detectors' own count rates to pin the background level, which makes the answer far more reliable; gives the probability for the purity number itself, plans how long to measure so that nine runs in ten succeed, and offers an early-stopping test whose error rates hold without knowing the emitter; models real detectors for heralded photon-pair sources; reads PicoQuant PTU files directly; and fixes several results that were wrong without a warning, among them the simulated effect of blinking, the treatment of histogram bins, and analysis windows reaching beyond the recorded data.</p>
...
<p>Optical filters are built by depositing many thin layers, and no deposition machine hits its targets exactly. The errors are not simple random scatter: they drift, they depend on the recipe, and it is the rare bad runs, not the average one, that decide how many manufactured devices actually work. fabtwin learns a machine's error behaviour from its own production logs, then redesigns the filter so that even the worst runs come out well. The optics behind it is exact, and every derivative it uses is worked out by hand and cross-checked against an independent method. Grown out of our FabGAN-ID paper (<em>IEEE Sensors Journal</em>, 2026), it accepts any lab's own logged data through a documented file format, and states plainly what a model learned from logs can and cannot promise. Latest release (0.7.0): designs can now be optimized for light arriving at an angle, in either polarization, through layers that absorb light, and against goals that are not a simple weighted sum, such as a pass/fail specification; the worst-case optimization gains an estimate that is not biased by the sample size; it can test whether the machine has drifted since its model was trained, flag designs unlike anything in the training data, and recover both thickness and material errors from spectra measured at several angles; its error ranges can now hold separately for each group of runs, or adapt as the process changes; it can redesign the remaining layers partway through a run to make up for errors already made; and it fixes a wrong reflectance for absorbing stacks with light hitting the substrate beyond the angle of total internal reflection.</p>

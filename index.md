...
<p class="intro-tagline">Adjunct Lecturer and Research Assistant, <strong>Department of Electrical and Electronic Engineering, BRAC University</strong>. I build first-principles, differentiable models of quantum and wide-bandgap devices, and use them to settle questions that measurement has posed but cannot answer on its own.</p>

The questions I take up are ones the literature has already recorded as open. Two magneto-optical probes of one GaN/AlN hole gas return heavy-hole masses differing by a third, a discrepancy their authors call a puzzle; a six-band calculation adjusting nothing shows it is an overdamped resonance rather than the band structure. A patterned dichalcogenide edge carries fixed charge and strain at once and no measurement had separated them; two phonons of the same spectrum do, giving 2.3 × 10<sup>12</sup> cm<sup>-2</sup> of edge electrons from Raman maps already published. A photonic-molecule route past the 3 dB squeezing limit was proposed but left undesigned; a model running from material data to detected quantum noise supplies it, at 8.5 dB. Every constant not computed comes from a published measurement, negative results are reported as such, and all code and data are released.
...
<a href="{{ '/research/#quantum-materials-mems' | relative_url }}"><strong>Electrons and phonons in 2D devices</strong></a>, conducted with Prof. Md. Mosaddequr Rahman. Carriers and lattice vibrations are followed through nitride and monolayer channels from the envelope functions and phonon eigenvectors upward, and are not allowed to stop at the band structure. The GaN/AlN calculation also finds that the measured lifetime ratios require forward-peaked long-range scattering rather than the short-range mechanism to which they have been attributed, and that no elastic mechanism accounts for both subbands at once, which places the standard two-carrier reading of the mobilities in question. Charge of the size the two phonons return accounts for a reported depletion-to-enhancement transition on thick oxide, yet is electrostatically invisible on a thin high-κ gate, where the damage halo left by the etch sets the width limit instead. In a WSe<sub>2</sub> ferroelectric transistor, one percent of biaxial compression more than doubles the hole mobility while moving the 1.24 V memory window by three percent, so strain and polarization act as independent design variables rather than competing ones.
